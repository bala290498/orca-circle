    ...
    >
      {(heading || description) && (
        <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
          {heading && (
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-4 max-w-4xl">
              {heading}
            </h2>
          )}
          {description && (
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-primary-100 border-2 border-primary-600 p-2" />
              </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-primary-600 via-primary-500 to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
